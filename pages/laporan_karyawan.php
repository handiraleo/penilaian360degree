<?php
	$id_periode = get_tahun_penilaian_id();
	if(isset($_GET['idta'])){
	...
	$sql = "SELECT 
				...
				tbnilai.level,
				tbnilai.jabatan,
				SUM( IF(tbnilai.nama_kompetensi = 'CoreCompetency', tbnilai.nilai, 0) ) AS 'CoreCompetency',
				SUM( IF(tbnilai.nama_kompetensi = 'Leadership', tbnilai.nilai, 0) ) AS 'Leadership',
				SUM( IF(tbnilai.nama_kompetensi = 'Soft', tbnilai.nilai, 0) ) AS 'Soft',
				SUM( IF(tbnilai.nama_kompetensi = 'Technical', tbnilai.nilai, 0) ) AS 'Technical'
			FROM 
			(SELECT 
				a.id_nilai, 
				h.nip as nip_dinilai,
				h.nama_karyawan as 'dinilai',
				e.nip as nip_penilai, 
				e.nama_karyawan as 'penilai',
				f.jabatan,
				f.level,
				c.id_kompetensi,
				c.nama_kompetensi,
	...
	$tot_arr['karyawan'] = 0;
	$tot_arr['sendiri'] = 0;
	$tot_CoreCompetency = 0;
	$tot_Leadership = 0;
	$tot_Technical = 0;
	$tot_Soft = 0;
	while($row = mysql_fetch_array($q)){
		$tot = 0;
		$pg = ($row['CoreCompetency']/10)*100;
		$kp = ($row['Leadership']/5)*100;
		$ss = ($row['Technical']/4)*100;
		$pr = ($row['Soft']/5)*100;


		$tot_CoreCompetency += $pg;
		$tot_Leadership += $kp;
		$tot_Technical += $ss;
		$tot_Soft += $pr;
		/* prestasi kinerja individu */
		$tot = ($pg*($b_CoreCompetency/100)) + ($kp*($b_Leadership/100)) + ($ss*($b_Technical/100)) + ($pr*($b_Soft/100));

		if($row['level']==2 || $row['level']==3){
			$tot_arr['atasan'] += $tot;
		}else if($row['level']==1 && $row['nip_penilai']!= $nip_user){
			$tot_arr['karyawan'] += $tot;
		}else{
			$tot_arr['sendiri'] += $tot;
		}
	}
	...
	$ak = ($tot_arr['atasan']*$set[0]) + ($tot_arr['karyawan']*$set[1]) + ($tot_arr['sendiri']*$set[2]);
	//$ak = ($tot_arr['atasan']*0.5) + ($tot_arr['karyawan']*0.3) + ($tot_arr['sendiri']*0.2);			
?>
<script>
	<?php
		echo "var data_bar = [";
		echo "{oleh: 'Atasan', nilai: $tot_arr[atasan] },";
		echo "{oleh: 'Rekan Kerja', nilai: $tot_arr[karyawan] },";
		echo "{oleh: 'Diri Sendiri', nilai: $tot_arr[sendiri] }";
		echo "];";


		echo "var data_kompetensi = [";
		echo "{oleh: 'CoreCompetency', nilai: ".number_format($tot_CoreCompetency / 6, 2)." },";
		echo "{oleh: 'Leadership', nilai: ".number_format($tot_Leadership / 6, 2)." },";
		echo "{oleh: 'Technical', nilai: ".number_format($tot_Technical / 6, 2)." },";
		echo "{oleh: 'Soft', nilai: ".number_format($tot_Soft / 6, 2)." }";
		echo "];";
	?>
</script>
...
<div class="card shadow mb-4">
    ...
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-danger"><i class="fa fa-table"></i> Pilih Daftar Periode</h6>
    </div>
				...
				<select class="form-control cb_periode" name="idta">
					<?php
						...
						while($row = mysql_fetch_array($q)){
							...
							if($row['status_periode']==1){
								echo "<option value='$row[id_periode]' $sel >$row[tahun_penilaian] $row[semester] (Aktif)</option>";
							}else{
								echo "<option value='$row[id_periode]' $sel >$row[tahun_penilaian] $row[semester]</option>";
							}
						}
					?>
				</select>
...
<div class="row">
	<div class="col-md-6">
		<div class="card shadow mb-4">
			<!-- /.card-header -->
			<div class="card-header py-3">
				<h6 class="m-0 font-weight-bold text-danger"><i class="fa fa-table"></i> Nilai Akhir</h6>
			</div>
	...
	<div class="col-md-6">
		<div class="card shadow mb-4">
			<!-- /.card-header -->
			<div class="card-header py-3">
				<h6 class="m-0 font-weight-bold text-danger"><i class="fa fa-table"></i> Nilai Perwakilan</h6>
			</div>
...
<div class="card shadow mb-4">
    <!-- /.card-header -->
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-danger"><i class="fa fa-table"></i> Nilai Perkompetensi</h6>
    </div>
